...
public class QueryBuilder {
    ...
    public PreparedStatement buildSql(KylinClient client,
                                      KylinClientHelper connection,
                                      String catalog,
                                      String schema,
                                      String table,
                                      List<KylinColumnHandle> columns,
                                      TupleDomain<ColumnHandle> tupleDomain)
            throws SQLException {
        StringBuilder sql = new StringBuilder();

        String columnNames = columns.stream()
                .map(KylinColumnHandle::getColumnName)
                .map(this::quote)
                .collect(joining(", "));
        ...
        PreparedStatement statement = null;//client.getPreparedStatement(connection, sql.toString());
    ...
    private List<String> toConjuncts(List<KylinColumnHandle> columns, TupleDomain<ColumnHandle> tupleDomain, List<TypeAndValue> accumulator) {
        ImmutableList.Builder<String> builder = ImmutableList.builder();
        for (KylinColumnHandle column : columns) {
            Type type = null;//column.getColumnType();
